...
- Viral Copy：用户给爆款 URL 时，基于 `note-detail` 输出结构级复刻建议，不逐字照抄，不复用原图。需要视觉复刻时默认直接调用 `gpt-image-2` 或宿主图像工具生成原创图片，不能只给提示词。拼豆图纸必须接入 `pindou-pattern` 生成真实色卡稿纸，不能编造现实珠子里不存在的颜色。
...
- 如果用户要求图纸：按“gpt-image-2 展示源图 -> pindou-pattern 真实色卡稿纸 -> gpt-image-2 成品效果图”三段闭环交付，并输出真实色号、色块建议、网格尺寸、备注说明；提示词只作为随附记录或无图像工具时的兜底。
- 拼豆图纸色卡规则：优先使用 `viral-copy` 上下文里的 `bead_palette_reference`，默认来源为 `https://拼豆.cn/palettes.json`、品牌 `MARD`。`pattern_sheet.palette` 必须写 `code/name/hex/usage`，不要写无法采购的抽象颜色。
- 运营呈现：公开晒图优先用展示源图和成品效果图；真实稿纸作为粉丝评论/私信索取时的承接资产。
