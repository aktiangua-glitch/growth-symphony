...
- Viral Copy：用户给爆款 URL 时，基于 `note-detail` 输出结构级复刻建议，不逐字照抄，不复用原图。需要视觉复刻时默认直接调用 `gpt-image-2` 或宿主图像工具生成原创图片/图纸，不能只给提示词。拼豆图纸必须引用真实色卡，不能编造现实珠子里不存在的颜色。
...
- 如果用户要求图纸：直接生成拼豆图纸/像素格稿图片，并输出真实色号、色块建议、网格尺寸、备注说明；提示词只作为随附记录或无图像工具时的兜底。
